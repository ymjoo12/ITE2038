...
namespace BPT
{
    ...
    NodePage load_node_page(int64_t table_id, pagenum_t page_number, bool& pinned, bool pin)
    {
        ...
        node_to_return = NodePage(BufferManager::read_page(table_id, page_number, pinned, pin));
    ...
    void set_node_page(int64_t table_id, pagenum_t page_number, NodePage& node, bool unpin, bool rollback)
    {
        if (LogUtil::DEBUG_MODE) {
            LogUtil::PrintMarker(__func__, "( page_number: " + std::to_string(page_number) + " )");
        ...
        // If rollback argument is true, unpin this page in buffer and skip to write
        if (rollback) {
            BufferManager::unpin_page(table_id, page_number, unpin);
            return;
        }
        ...
        BufferManager::write_page(table_id, page_number, node, unpin);
    }
    ...
    pagenum_t make_node_page(int64_t table_id, bool is_leaf)
    {
        ...
        // allocate new page
        new_page_number = BufferManager::alloc_page(table_id);

        // create new node page and initialize the node page in buffer
        new_node = NodePage(is_leaf);
        set_node_page(table_id, new_page_number, new_node, false);

        return new_page_number;
    }

    void free_node_page(int64_t table_id, pagenum_t page_number, bool unpin)
    {
        if (LogUtil::DEBUG_MODE) LogUtil::PrintMarker(__func__);

        // free given page
        BufferManager::free_page(table_id, page_number, unpin);
    }
    ...
    pagenum_t get_root_page(int64_t table_id)
    {
        ...
        header = HeaderPage(BufferManager::read_page(table_id, 0, pinned));
    ...
    void set_root_page(int64_t table_id, pagenum_t root)
    {
        ...
        // Update header page
        header = HeaderPage(BufferManager::read_page(table_id, 0, pinned, true));
        header.root_page_number = root;
        BufferManager::write_page(table_id, 0, header, pinned);
    }
    ...
    void set_parent_page(int64_t table_id, pagenum_t child, pagenum_t parent)
    {
        ...
        child_node = load_node_page(table_id, child, pinned, true);
        child_node.header.parent_page_number = parent;
        set_node_page(table_id, child, child_node, pinned);
    }
    ...
    void set_parent_pages(int64_t table_id, pagenum_t parent, int start_point) 
    {
        ...
        parent_node = load_node_page(table_id, parent, pinned);
    ...
    // Update metadata of node page (header, offset)
    ...
    pagenum_t find_leaf(int64_t table_id, pagenum_t root, int64_t key)
    {
        ...
        if (root == 0) return 0;
        node = load_node_page(table_id, root, pinned);
        ...
        while (!node.header.is_leaf) {
            ...
            node = load_node_page(table_id, leaf, pinned);
        }
    ...
    int find(int64_t table_id, pagenum_t root, int64_t key, std::string& value)
    {
        ...
        leaf_node = load_node_page(table_id, leaf, pinned);
    ...
    int insert_into_leaf(int64_t table_id, pagenum_t leaf, int64_t key, std::string value)
    {
        ...
        leaf_node = load_node_page(table_id, leaf, pinned, true);
        if (!leaf_node.header.is_leaf) {
            set_node_page(table_id, leaf, leaf_node, pinned, true);
            return FLAG_FAIL;
        }
        ...
        // set the node
        set_node_page(table_id, leaf, leaf_node, pinned);
        return FLAG_SUCCESS;
    }
    ...
    int insert_into_leaf_after_splitting(int64_t table_id, pagenum_t leaf, int64_t key, std::string value)
    {
        ...
        // load leaf node
        leaf_node = load_node_page(table_id, leaf, pinned_l, true);
        if (!leaf_node.header.is_leaf) {
            set_node_page(table_id, leaf, leaf_node, pinned_l, true);
            return FLAG_FAIL;
        }
        ...
        new_leaf_node = load_node_page(table_id, new_leaf, pinned_n, true);

        // split array of slots
        flag = split_leaf_slots(new_leaf_node.slots, leaf_node.slots, new_key);
        if (flag) {
            free_node_page(table_id, new_leaf, pinned_n);
            return FLAG_FAIL;
        };
        ...
        // set the node pages
        set_node_page(table_id, leaf, leaf_node, pinned_l);
        set_node_page(table_id, new_leaf, new_leaf_node, pinned_n);
    ...
    int insert_into_internal(int64_t table_id, pagenum_t internal, pagenum_t left, int64_t key, pagenum_t right)
    {
        ...
        internal_node = load_node_page(table_id, internal, pinned, true);
        if (internal_node.header.is_leaf) {
            set_node_page(table_id, internal, internal_node, pinned, true);
            return FLAG_FAIL;
        }
        ...
        // set the node
        set_node_page(table_id, internal, internal_node, pinned);
        return FLAG_SUCCESS;
    }
    ...
    int insert_into_internal_after_splitting(int64_t table_id, pagenum_t internal, pagenum_t left, int64_t key, pagenum_t right) 
    {
        ...
        // load internal node
        internal_node = load_node_page(table_id, internal, pinned_i, true);
        if (internal_node.header.is_leaf) {
            set_node_page(table_id, internal, internal_node, pinned_i, true);
            return FLAG_FAIL;
        }
        ...
        new_internal_node = load_node_page(table_id, new_internal, pinned_n, true);

        // split array of edges
        flag = split_internal_edges(new_internal_node, internal_node.edges, new_key);
        if (flag) {
            free_node_page(table_id, new_internal, pinned_n);
            return FLAG_FAIL;
        };
        ...
        // set the node pages
        set_node_page(table_id, internal, internal_node, pinned_i);
        set_node_page(table_id, new_internal, new_internal_node, pinned_n);
    ...
    int insert_into_parent(int64_t table_id, pagenum_t left, int64_t key, pagenum_t right)
    {
        ...
        // load left node
        left_node = load_node_page(table_id, left, pinned_l);
        ...
        parent_node = load_node_page(table_id, parent, pinned_p);
    ...
    int insert_into_new_root(int64_t table_id, pagenum_t left, int64_t key, pagenum_t right)
    {
        ...
        new_root_node = load_node_page(table_id, new_root, pinned, true);
        ...
        set_node_page(table_id, new_root, new_root_node, pinned);
    ...
    int remove_entry_from_node(int64_t table_id, pagenum_t key_page, int64_t key)
    {
        ...
        key_node = load_node_page(table_id, key_page, pinned, true);
        ...
        // set the node
        set_node_page(table_id, key_page, key_node, pinned);
        return FLAG_SUCCESS;
    }
    ...
    int adjust_root(int64_t table_id, pagenum_t root)
    {
        ...
        // load root page
        root_node = load_node_page(table_id, root, pinned, true);
        if (root_node.header.parent_page_number != 0) {
            set_node_page(table_id, root, root_node, pinned, true);
            return FLAG_FAIL;
        }
        ...
        // free old root page
        free_node_page(table_id, root, pinned);
        return FLAG_SUCCESS;
    }
    ...
    {
        bool pinned_l, pinned_r;
        int insertion_index;
        pagenum_t parent;
        NodePage left_node, right_node;

        // load node pages and get data
        left_node = load_node_page(table_id, left, pinned_l, true);
        right_node = load_node_page(table_id, right, pinned_r, true);
        ...
        if (left_node.header.is_leaf) {
            ...
            set_node_page(table_id, left, left_node, pinned_l);
        } else {
            // Case: internal node
        ...
        free_node_page(table_id, right, pinned_r);
    ...
    {
        bool pinned_l, pinned_r, pinned_p;
        pagenum_t parent;
        ...
        parent = left_node.header.parent_page_number;
        parent_node = load_node_page(table_id, parent, pinned_p, true);
        if (parent_node.edges[prime_key_index].page_number != right) {
            set_node_page(table_id, left, left_node, pinned_l, true);
            set_node_page(table_id, right, right_node, pinned_r, true);
            set_node_page(table_id, parent, parent_node, pinned_p, true);
            return FLAG_FAIL;
        }
        ...
        set_node_page(table_id, parent, parent_node, pinned_p);

        // Save redistributed nodes
        set_node_page(table_id, left, left_node, pinned_l);
        set_node_page(table_id, right, right_node, pinned_r);
    ...
    {
        bool pinned_k, pinned_p, pinned_n;
        int flag, key_index;
        ...
        key_node = load_node_page(table_id, key_page, pinned_k);
        ...
        neighbor_node = load_node_page(table_id, neighbor, pinned_n);
        if (
    ...
    void destroy_tree_nodes(int64_t table_id, pagenum_t page_number)
    {
        ...
        // load given page
        node = load_node_page(table_id, page_number, pinned);

        // free node pages
        if (node.header.is_leaf) {
            free_node_page(table_id, page_number, pinned);
        } else {
