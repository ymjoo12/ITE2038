...
{
    this->next_free_page_number = 0;
    this->num_of_pages = 0;
    this->root_page_number = 0;
...
// constructors
Record::Record()
    : KeyPair(0), size(0), offset(0)
{}

Record::Record(const int64_t key, const std::string value, const uint16_t offset)
    : KeyPair(key), value(value), size(value.size()), offset(offset)
{}

Record::Record(const Record& copy)
    : KeyPair(copy.key), size(copy.size), offset(copy.offset), value(copy.value)
{}
