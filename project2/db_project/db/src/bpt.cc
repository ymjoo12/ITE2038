...
namespace BPT
{
    ...
    NodePage load_node_page(int64_t table_id, pagenum_t page_number)
    {
    ...
    void flush_node_page(int64_t table_id, pagenum_t page_number, NodePage& node)
    {
    ...
    pagenum_t make_node_page(int64_t table_id, bool is_leaf)
    {
        ...
        flush_node_page(table_id, new_page_number, new_node);
    ...
    void set_parent_page(int64_t table_id, pagenum_t child, pagenum_t parent)
    {
        ...
        child_node = load_node_page(table_id, child);
        child_node.header.parent_page_number = parent;
        flush_node_page(table_id, child, child_node);
    }
    ...
    void update_leaf_node(NodePage& leaf_node, int start_point)
    {
        uint16_t offset;
    ...
    void update_internal_node(NodePage& internal_node)
    {
        // Check the node is internal
    ...
    int insert_into_leaf(int64_t table_id, pagenum_t leaf, int64_t key, std::string value)
    {
        ...
        update_leaf_node(leaf_node, index);

        // flush the node
        flush_node_page(table_id, leaf, leaf_node);
    ...
    int insert_into_leaf_after_splitting(int64_t table_id, pagenum_t leaf, int64_t key, std::string value)
    {
        ...
        update_leaf_node(leaf_node);
        update_leaf_node(new_leaf_node);

        // flush the node pages
        flush_node_page(table_id, leaf, leaf_node);
        flush_node_page(table_id, new_leaf, new_leaf_node);
    ...
    int insert_into_internal(int64_t table_id, pagenum_t internal, pagenum_t left, int64_t key, pagenum_t right)
    {
        ...
        update_internal_node(internal_node);

        // flush the node
        flush_node_page(table_id, internal, internal_node);
    ...
    int insert_into_internal_after_splitting(int64_t table_id, pagenum_t internal, pagenum_t left, int64_t key, pagenum_t right) 
    {
        ...
        update_internal_node(internal_node);
        update_internal_node(new_internal_node);

        // flush the node pages
        flush_node_page(table_id, internal, internal_node);
        flush_node_page(table_id, new_internal, new_internal_node);
    ...
    int insert_into_new_root(int64_t table_id, pagenum_t left, int64_t key, pagenum_t right)
    {
        ...
        flush_node_page(table_id, new_root, new_root_node);
    ...
    int remove_entry_from_node(int64_t table_id, pagenum_t key_page, int64_t key)
    {
        ...
        if (key_node.header.is_leaf) {
            ...
            if (index < 0) return FLAG_FAIL;
            update_leaf_node(key_node, index);
        } else {
            index = delete_node_key(key_node.edges, key);
            update_internal_node(key_node);
        }

        // flush the node
        flush_node_page(table_id, key_page, key_node);
    ...
    int merge_nodes(int64_t table_id, pagenum_t root, pagenum_t left, pagenum_t right, int64_t prime_key)
    {
        ...
        if (left_node.header.is_leaf) {
            ...
            update_leaf_node(left_node, insertion_index);
            flush_node_page(table_id, left, left_node);
        } else {
            // Case: internal node
            ...
            update_internal_node(left_node);
            flush_node_page(table_id, left, left_node);
        ...
        if (left_node.header.is_leaf) {
            ...
            if (left_node.header.amount_of_free_space < right_node.header.amount_of_free_space) {
                ...
                while (right_node.header.amount_of_free_space >= RECORD_THRESHOLD) {
                    ...
                    update_leaf_node(right_node);
                }
                update_leaf_node(left_node, left_node.slots.size());
            } else {
                // Case: number of left records < number of right records
                while (left_node.header.amount_of_free_space >= RECORD_THRESHOLD) {
                    ...
                    update_leaf_node(left_node, left_node.header.number_of_keys);
                }
                update_leaf_node(right_node);
            }
            // Update prime key
            ...
            update_internal_node(left_node);
            update_internal_node(right_node);
        }

        // Update and save parent node
        parent_node.edges[prime_key_index].key = prime_key;
        flush_node_page(table_id, parent, parent_node);

        // Save redistributed nodes
        flush_node_page(table_id, left, left_node);
        flush_node_page(table_id, right, right_node);
