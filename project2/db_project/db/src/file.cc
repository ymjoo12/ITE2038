...
{    
    ...
    page_number_to_alloc = page_buffer.next_free_page_number;
    ...
    // Update new allocated page (Initialize the page_number_to_alloc to the page number itself)
    page_buffer.next_free_page_number = page_number_to_alloc;
    FileUtil::write_block(fd, &page_buffer, PAGE_SIZE, PAGE_SIZE * page_number_to_alloc);

    // Update header page
    FileUtil::read_block(fd, &page_buffer, PAGE_SIZE);
    page_buffer.next_free_page_number = new_next_free_page_number;
...
void file_free_page(int64_t table_id, pagenum_t pagenum)
{
    ...
    // Update new free page (not initialize reserved space)
    FileUtil::read_block(fd, &page_buffer, PAGE_SIZE, PAGE_SIZE * pagenum);
...
void file_read_page(int64_t table_id, pagenum_t pagenum, page_t* dest)
{
    ...
    // Check the page has valid page number
    if (!FileUtil::is_valid_page_number(fd, pagenum)) {
        std::cout << "[file_read_page] Tried to read invalid page ";
        std::cout << "( table_id: " << table_id << ", page_number: " << pagenum << " )" << std::endl;
        return;
    }
...
void file_write_page(int64_t table_id, pagenum_t pagenum, const page_t* src)
{
    ...
    // Check the page has valid page number
    if (!FileUtil::is_valid_page_number(fd, pagenum)) {
        std::cout << "[file_write_page] Tried to write invalid page ";
        std::cout << "( table_id: " << table_id << ", page_number: " << pagenum << " )" << std::endl;
        return;
    }
