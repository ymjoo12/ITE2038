...
namespace TestUtil 
{
    ...
    bool IsAllocatedPage(int64_t table_id, int page_num)
    {
        if (page_num <= 0) return false;

        HeaderPage page_buffer;
        int table_file = opened_tables.getFileDesc(table_id);
        pread(table_file, &page_buffer, PAGE_SIZE, PAGE_SIZE * page_num);
        return page_buffer.next_free_page_number == page_num;
    }
    ...
    {
        int table_file = opened_tables.getFileDesc(table_id);
        int flag = fcntl(table_file, F_GETFL) < 0;
        return flag;
    }
    ...
    void LoadDB(const std::string filename)
    {
        ...
        file_open_table_file(filename.c_str());
