...
{
    if (LogUtil::DEBUG_MODE) std::cout << "[get_page] ( table_id: " << table_id << ", pg_num: " << pg_num << " )" << std::endl;
...
void BufferManager::set_dirty_page(int index, const page_t& pg_img)
{
    if (LogUtil::DEBUG_MODE) std::cout << "[set_dirty_page] ( index: " << index << " )" << std::endl;
...
{
    if (LogUtil::DEBUG_MODE) std::cout << "[pin_page] ( table_id: " << table_id << ", pg_num: " << pg_num << " )" << std::endl;
...
namespace BUF
{
    ...
    /// Page controllers
    page_t read_page(int64_t table_id, pagenum_t pg_num, int& pin_id, bool pin)
    {
        page_t pg_img;
    ...
    {
        // Write page image
        buffer.set_dirty_page(pin_id, pg_img);
    }
