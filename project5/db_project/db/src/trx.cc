...
int TransactionManager::acquire_lock(int64_t table_id, pagenum_t page_id, int64_t key, int trx_id, int lock_mode)
{
    int flag, old_trx_id, record_id;
    lock_t* lock_obj;
    page_key_t page;
...
namespace TRX
{
    ...
    // Acquire lock
    int acquire_lock(int64_t table_id, pagenum_t page_id, int64_t key, int trx_id, int lock_mode) 
    {
        int flag;

        // Acquire the lock for the transaction
        flag = trx_manager.acquire_lock(table_id, page_id, key, trx_id, lock_mode);
