...
transfer_thread_func(void* arg)
{
	...
	for (int i = 0; i < TRANSFER_COUNT; i++) {
		...
		/* Acquire lock!! */
		source_lock = lock_acquire(source_table_id, source_record_id, 0, 0, 0);
		...
		/* Acquire lock!! */
		destination_lock =
			lock_acquire(destination_table_id, destination_record_id, 0, 0, 0);
		...
		for (int table_id = 0; table_id < TABLE_NUMBER; table_id++) {
			for (int record_id = 0; record_id < RECORD_NUMBER; record_id++) {
				...
				lock_array[table_id][record_id] =
					lock_acquire(table_id, record_id, 0, 0, 0);
